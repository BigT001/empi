...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[DATABASE RESET] Deleted ${ordersDeleted.deletedCount} orders at ${new Date().toISOString()}`);

    return NextResponse.json({
      success: true,
      message: "Database reset complete - All users, orders, and transaction histories deleted",
      deletedCounts: {
      ...
      warning: "⚠️ This action deleted all user, order, and transaction data and cannot be undone"
    });
  } catch (error) {
